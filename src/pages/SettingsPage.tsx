...
import { toggleCategory } from "../utils/uiSemantics";
...
function categoryState(providers: ProviderStatus[], category: MetricCategory): CapabilityState | "notReported" {
  const items = providers.flatMap((provider) => provider.capabilities.filter((capability) => capability.category === category));
  if (items.some((item) => item.state === "failed")) return "failed";
  if (items.some((item) => item.state === "unsupported")) return "unsupported";
  if (items.some((item) => item.state === "supportedDisabled")) return "supportedDisabled";
  if (items.some((item) => item.state === "supportedEnabled")) return "supportedEnabled";
  return "notReported";
}
