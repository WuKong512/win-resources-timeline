...
export function AppResourceHistoryChart({ points }: { points: AppResourceHistoryPoint[] }) {
  ...
  useEffect(() => {
    ...
    chart.setOption({
      ...
      tooltip: { trigger: "axis", backgroundColor: card, borderColor: border, borderWidth: 1, padding: [10, 12], textStyle: { color: foreground, fontSize: 12 }, extraCssText: "box-shadow: 0 18px 48px rgba(0,0,0,.24); border-radius: 8px;" },
      ...
      series: [
        { name: "CPU", type: "line", xAxisIndex: 0, yAxisIndex: 0, showSymbol: false, connectNulls: false, lineStyle: { width: 1.8 }, emphasis: { focus: "series" }, tooltip: { valueFormatter: (value: number) => `${value.toFixed(1)}%` }, data: points.map((point) => [point.timestampMs, point.cpuPercent]) },
        { name: t("memory"), type: "line", xAxisIndex: 0, yAxisIndex: 1, showSymbol: false, connectNulls: false, lineStyle: { width: 1.8 }, emphasis: { focus: "series" }, tooltip: { valueFormatter: (value: number) => formatBytes(value, language) }, data: points.map((point) => [point.timestampMs, point.memoryUsedBytes]) },
        { name: t("ioRead"), type: "line", xAxisIndex: 1, yAxisIndex: 2, showSymbol: false, connectNulls: false, lineStyle: { width: 1.6 }, emphasis: { focus: "series" }, tooltip: { valueFormatter: (value: number) => `${formatBytes(value, language)}/s` }, data: points.map((point) => [point.timestampMs, point.ioReadBytesPerSec]) },
        { name: t("ioWrite"), type: "line", xAxisIndex: 1, yAxisIndex: 2, showSymbol: false, connectNulls: false, lineStyle: { width: 1.6 }, emphasis: { focus: "series" }, tooltip: { valueFormatter: (value: number) => `${formatBytes(value, language)}/s` }, data: points.map((point) => [point.timestampMs, point.ioWriteBytesPerSec]) }
      ]
    });
    const resize = () => chart.resize();
    window.addEventListener("resize", resize);
    return () => { window.removeEventListener("resize", resize); chart.dispose(); };
  }, [language, points, t, resolvedTheme]);
