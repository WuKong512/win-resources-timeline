...
当前已准备新的 `amd-uprof-static-api-hold-fixture.exe`，保留原 M1 的官方 `AMDPowerProfileAPI.lib` static import 策略和 `AMDPowerProfileAPI.dll -> AMDSysUtils.dll -> CXLBaseTools.dll` 依赖链，但使用 checked synchronous `WriteFile` markers，并在正常 return 前持有 3,000 ms。新二进制 SHA 为 `B680E7761FC3E64193E7140B57326154A64AB702C62763C7693EA97234DC1676`，原 M1 SHA 未被覆盖。非 AMD synthetic wrapper qualification、x64/PE direct-import assertion、no-AMD-API-from-main audit 和 PowerShell syntax check 已通过；AMD-linked hold fixture 尚未运行。下一步仅需用户在手动 elevated x64 PowerShell 中执行一次 `ADMIN_STATIC_LIFETIME_DISCRIMINATOR_REQUIRED` 命令，以区分 `STARTUP` 与 `SHUTDOWN_OR_DETACH` failure family；不得重跑 M1、B1、profiling/sampling 或开始 `CPU-SENSOR-AMD-PROVIDER-DESIGN`。详见 [`docs/measurements/cpu-sensor-amd-static-fixture-lifetime-discriminator.md`](../measurements/cpu-sensor-amd-static-fixture-lifetime-discriminator.md)。
