...
4. 在不破坏现有用户数据的前提下，将 schema v6 演进到 v7，再由独立的 PR-04A 补齐 v8 GPU storage contract。
...
| [data-model-v7.md](./data-model-v7.md) | v7 基线、v8 GPU storage contract、语义和查询模型 |
...
| [migration-strategy.md](./migration-strategy.md) | v6 → v7、v7 → v8 迁移、校验和恢复 |
...
- Spike-01 short-term implementation admission、default-enable admission、support-matrix evidence 和 release/stability gate 分层判断；24 小时 soak、AMD/Intel validation 和完整硬件矩阵不阻塞 PR-04A storage contract，但仍是后续发布/支持声明门槛。
