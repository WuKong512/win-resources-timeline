...
- `CollectionPlan` 由持久化 settings、descriptor capabilities 和采样 policy 确定性编译。ProviderHost 保留用户 intent 的 desired plan，并在 capability/policy 变化时重新生成 effective plan；runtime observed lifecycle 独立记录实际的 stopped/running/failed/paused 状态。它只在启动、settings reload 或能力变化时重建，采样热路径只消费已编译计划。
...
- `ProviderHost` 通过 bounded worker executor 等待 provider 调用；pending call 带 operation/generation，late lifecycle completion 按当前 user intent/plan reconcile，旧 probe/sample 结果不能覆盖新 generation，过期 sample payload 不进入 FrameWriter。单个 provider 的 startup/reconfigure/sample failure 只更新其 bounded health/backoff 状态，超时调用被隔离，其他 provider 和 foreground/computer-state timeline 继续运行。普通 probe/start/reconfigure/disable stop/pause stop/resume start 为每个 provider 独立生成 operation budget；只有 shutdown 复用现有 absolute deadline，provider stop 结果可报告 failure/timeout，不得无限阻塞或无限重试。
- current-generation 的 late Probe 或 lifecycle capability outcome 改变 capability truth 时，Host 从 desired plan 重新计算 effective plan，而不是只从已有 effective plan 单向删除类别；因此 capability 恢复可以重新进入 active plan，用户 disable、pause、shutdown 和更新后的 settings generation 仍具有更高 intent 优先级。
