...
PR-07 mandatory acceptance 使用 multi-session extended native qualification：至少 3 个独立 native session、每个 >=10 小时、至少一个 >=12 小时、aggregate valid native runtime >=32 小时，并 collectively 覆盖 long idle/background、normal interactive、local-midnight rollover、动态 Provider/category 启停、数据库忙/恢复、clean process shutdown/reopen、schema/integrity continuity、bounded resource behavior 和 validated-profile performance/storage budget。continuous 24-hour soak 保留为 optional extended qualification，不是 mandatory blocker。

该 multi-session model aligned with the application's expected real-world duty cycle：Windows 启动后通常运行十多个小时，再经历 sleep/shutdown 并在下一 session 重新启动、reopen DB，多日重复。它同时观察 within-session slow resource growth、repeated startup/shutdown、WAL checkpoint/recovery、Provider lifecycle recreation 和 stale native object/mutex risk；单次 >=10 小时与 >=12 小时约束仍保留对 memory leak、handle/thread growth、queue accumulation、WAL runaway 和 retry loop 的敏感性。性能回归基于同一采样配置比较，不能混用配置得出结论。

Real Windows sleep/wake 是当前 validated AMD/NVIDIA desktop profile 的 `DEFERRED POWER-STATE / COMPATIBILITY QUALIFICATION`：不是 mandatory PR-07B blocker，也不是 correctness PASS。任何观察到的异常必须保留为 evidence，不得归因于 Resource Timeline、Windows 或硬件；当前异常 attribution 为 `UNKNOWN`，后续 compatibility declaration gate 独立处理，并继续标记 `DEFERRED — NON-BLOCKING`。
